...
const ADMIN_RATE_LIMIT_KEY = "admin"
...
function authenticateAdmin(req: Request): Response | null {
...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    ...
    const unauthorized = authenticateAdmin(req)
    if (unauthorized) {
      return unauthorized
    }

    const url = new URL(req.url)
    const status = parseCorrectionStatus(
      url.searchParams.get("status") ?? "pending"
    )

    if (!status) {
      return jsonResponse(
        { error: "Invalid correction status." },
        { status: 400 }
      )
    }
...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    ...
    try {
      const correction = await ctx.runMutation(
        internal.corrections.updateStatus,
        {
