...
export function VyuoPageClient() {
  ...
  return (
    <main className="min-h-screen bg-white text-brand-ink">
      ...
      <div className="mx-auto grid max-w-[1280px] gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[16.5rem_1fr] lg:gap-10 lg:px-8 lg:py-10">
        <VyuoFilters
        ...
        <section className="min-w-0">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <label className="flex items-center gap-2 text-[12.5px] text-brand-ink/65">
              <span>Panga kwa:</span>
              <select
                value={sort}
                onChange={(event) => setSort(event.target.value)}
                className="h-9 rounded-lg border border-brand-ink/15 bg-white px-2.5 text-[13px] outline-none focus:border-brand-blue"
              >
...
function VyuoHero({
  ...
  return (
    <section className="border-b border-brand-ink/8 bg-[#fbfaf6]">
      <div className="mx-auto grid max-w-[1280px] gap-10 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_22rem] lg:gap-16 lg:px-8 lg:py-16">
        <div className="min-w-0">
          ...
          <h1 className="mt-4 text-[38px] font-bold leading-[1.05] tracking-tight sm:text-[52px]">
            Tafuta vyuo na colleges
            <br />
            <span className="text-brand-ink/55">zinazokufaa.</span>
          </h1>
          <p className="mt-4 max-w-xl text-[15.5px] leading-7 text-brand-ink/65">
            Gundua taasisi kulingana na eneo, aina ya umiliki, ngazi ya kozi, na programu wanazotoa.
          </p>

          <form
            onSubmit={(event) => event.preventDefault()}
            className="mt-7 flex max-w-2xl flex-col gap-2 rounded-3xl border border-brand-ink/10 bg-white p-2 shadow-[0_18px_38px_-22px_rgba(15,15,18,0.25)] sm:flex-row sm:items-center sm:rounded-full"
          >
            ...
            <button
              type="submit"
              className="h-11 shrink-0 rounded-full bg-brand-blue px-7 text-[14px] font-semibold text-white transition hover:bg-brand-blue-deep"
            >
              Tafuta
            </button>
          </form>
